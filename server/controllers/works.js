    ...
    if (project.members[i].id === req.user.id) {
      if (project.members[i].access === "Owner" || project.members[i].access === "Admin" || project.members[i].access === "Editor") {
        ...
        const mailOptions = {
          ...
          text: `Hi ${req.body.name}, you have been invited to join project ${project.title} by ${user.name}. Please click on the link to join the project. http://localhost:8700/api/project/invite/${req.params.id}/${req.body.id}`,
        };
...
export const updatework =  async (req, res) => {
  const { workId,taskId, newStatus } = req.body;
console.log(taskId);
  ...
  try {
    ...
    if (work&&task) {
      // console.log(work.taskId.status);
...
export const updateprojectstatus =  async (req, res) => {
  ...
  try {
    const work = await Project.findById(workId);
    if (work) {
      work.status = newStatus;
      await work.save(); 
      res.status(200).json({ message: 'Task status updated successfully' });
